...
object Generator {
	...
	private fun prepareProjects(projectIndex: Int, list: List<List<String>>): HashMap<String, LinkedList<ProjectEntry>> =
		list.map { it[projectIndex] }
			.toHashSet()
			.let {
				HashMap<String, LinkedList<ProjectEntry>>().apply {
					it.forEach { put(it, LinkedList()) }
				}
			}
